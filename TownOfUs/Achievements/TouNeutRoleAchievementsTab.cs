...
public class TouNeutRoleAchievementsTab : AchievementsTab
{
    ...
    public CountBundleAchievement SnakeEyes { get; set; } = new("Snake Eyes",
        "Petrify 10 players with Stone Gaze.", TouNeutAssets.StoneGazeSprite, TouRoleIcons.Medusa, 0, 10);

    public BaseBundleAchievement ChangeOfHeart { get; set; } = new("Change of Heart",
        "Convert a player when less than 5 players remain.", TouNeutAssets.BiteSprite, TouRoleIcons.Vampire);
    ...
    public BaseBundleAchievement FullCourseMeal { get; set; } = new("Full-Course Meal",
        "Feed a Mini body and a Giant body in the same game.", TouNeutAssets.ChefServeTurkeySprite, TouRoleIcons.Chef, 1);
