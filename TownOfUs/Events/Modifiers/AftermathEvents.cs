...
public static class AftermathEvents
{
    ...
    public static void AftermathDeathEvent(AfterMurderEvent @event)
    {
        ...
        var text = MiraLocaleManager.Get("TownOfUsMira.Modifier.AftermathTriggeredNotif").Replace("<player>", @event.Target.Data.PlayerName);

        var notif1 = Helpers.CreateAndShowNotification(
            $"<b>{text.Replace("<modifier>", $"{TownOfUsColors.Aftermath.ToTextColor()}{MiraLocaleManager.Get("TownOfUsMira.Modifier.Aftermath")}</color>")}</b>",
