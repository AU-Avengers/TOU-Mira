...
public sealed class LookoutWatchedModifier(PlayerControl lookout) : BaseModifier
{
    ...
    public override void OnMeetingStart()
    {
        ...
        var title = $"<color=#{TownOfUsColors.Lookout.ToHtmlStringRGBA()}>{MiraLocaleManager.Get("TouRoleLookoutFeedbackTitle")}</color>";
        var msg = MiraLocaleManager.Get("TouRoleLookoutNoInteractionFeedback").Replace("<player>", Player.Data.PlayerName);
        ...
        if (SeenPlayers.Count != 0)
        {
            ...
            var message = new StringBuilder($"{MiraLocaleManager.Get(showRoles ? "TouRoleLookoutInteractionFeedback" : "TouRoleLookoutAltInteractionFeedback").Replace("<player>", Player.Data.PlayerName)}:\n");
