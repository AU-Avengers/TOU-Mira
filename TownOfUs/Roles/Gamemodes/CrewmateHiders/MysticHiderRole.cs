...
namespace TownOfUs.Roles.Gamemodes.CrewmateHiders;

public sealed class MysticHiderRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITownOfUsRole, IWikiDiscoverable
{
    public string LocaleKey => "HiderMystic";
    ...
    {
        if (!Player || !Player.AmOwner || Player.Data.Role is not MysticHiderRole || MeetingHud.Instance)
        {
            return;
        }
