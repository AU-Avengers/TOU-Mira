...
public sealed class ImitatorRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITownOfUsRole, IWikiDiscoverable, IDoomable
{
    ...
    public float ShowAbilitiesTab(Transform abilityTemplate, Transform abilityTemplateLong, Transform abilityScroller)
    {
        var listOfAbilities = new List<GameObject>();
            ...
            text.text =
                $"<font=\"LiberationSans SDF\" material=\"LiberationSans SDF - Chat Message Masked\">{MiraLocaleManager.Get($"TouRole{IdPart}CrewmateImitation")}</font>";
            desc.text =
                $"<font=\"LiberationSans SDF\" material=\"LiberationSans SDF - Chat Message Masked\">{MiraLocaleManager.Get($"TouRole{IdPart}CrewmateImitationWikiDescription")}</font>";
            ...
            listOfAbilities.Add(AddTabInfo(MiraLocaleManager.Get($"TouRole{IdPart}NeutralCounterparts"), neutEquivalents, abilityTemplateLong, 1.025f, newSubObject.transform));
            listOfAbilities.Add(AddTabInfo(MiraLocaleManager.Get($"TouRole{IdPart}ImpostorCounterparts"), impEquivalents, abilityTemplateLong, -1.025f, newSubObject.transform));
